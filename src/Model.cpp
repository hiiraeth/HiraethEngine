...
Mesh Model::processMesh(aiMesh *mesh, const aiScene *scene)
{
    ...
    for(unsigned int i = 0; i < mesh->mNumVertices; i++)
    {
        ...
        // Texture Coordinates (Assimp allows up to 8 different texture coordinates per vertex)
    ...
    if(mesh->mMaterialIndex >= 0)
    {
        ...
        // We will extract diffuse and specular maps here
	...
	m.BaseDiffuse = diffuseColor;
	m.BaseSpecular = specularColor;
	m.BaseShininess = shiny;
	return m;
}
...
unsigned int Model::TextureFromFile(const char *path, const std::string &directory)
{
    ...
    // Extract just the filename to prevent absolute path errors from the .mtl file
    ...
    {
        LOG_ERROR("ASSIMP: Texture failed to load at path: " << path << std::endl);
        stbi_image_free(data);
    }
